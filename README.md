...
O Quorum coloca agentes de fornecedores diferentes sobre o mesmo problema, confina as ferramentas ao projeto, registra o que cada lado verificou e entrega uma síntese única. Um provedor sem instalação, login, modelo ou quota fica como `skipped`; os demais continuam.

## Destaques

- Rede plugável com Claude Code, Codex, NVIDIA/Nemotron, DeepSeek, Kimi e Antigravity/Gemini.
...
- Painel premium e responsivo com diagnóstico de modelos, seleção de conselheiros e disponibilidade em tempo real.
- DeepSeek e Kimi usam as ferramentas confinadas do Quorum; Antigravity revisa relatórios em um diretório isolado.

## Provedores

| Provedor | Transporte | Papel padrão | Perfil de uso |
|---|---|---|---|
| Claude | `claude -p` | especialistas + líder | sessão local autenticada |
| OpenAI/Codex | `codex exec` | especialistas + juiz | sessão local autenticada |
| NVIDIA/Nemotron | NIM compatível com OpenAI | documentação independente | free tier, exige `NVIDIA_API_KEY` |
| DeepSeek | Ollama `deepseek-r1:8b` | segurança, performance e tokens | local, sem custo por token |
| Kimi | Ollama Cloud `kimi-k2.6:cloud` | qualidade e design | plano Free, sujeito à quota |
| Antigravity/Gemini | `agy` headless | revisão cruzada pós-juízes | quota da conta Antigravity |

“Gratuito” significa execução no hardware local ou uma quota gratuita oferecida pelo provedor. Quotas e modelos cloud podem mudar; rode o diagnóstico antes de uma revisão importante.
...
|---|---|
| `NVIDIA_API_KEY` | Para habilitar o Grupo NVIDIA/Nemotron. |
...
1. O líder inspeciona rapidamente o escopo e monta um brief comum.
2. Claude, Codex, Nemotron, DeepSeek e Kimi trabalham em paralelo quando disponíveis.
3. Cada grupo principal consolida e verifica seus achados.
4. Antigravity/Gemini revisa os relatórios consolidados em um workspace isolado.
5. O líder cruza evidências, mantém divergências decisórias abertas e grava `runs/<run-id>/FINAL_REPORT.md`.
...
```bash
node orchestrate.js --scope /projeto --task "revisar" --run-id run-1 --out runs/run-1 --providers '["deepseek-local","antigravity-free"]'
```
